\begin{tikzpicture}[
    font=\normalsize,
    >=Latex,
    arrow/.style={->, thick},
    box/.style={draw, rounded corners=1pt, align=center, minimum width=3.2cm, minimum height=1.0cm},
    widebox/.style={draw, rounded corners=1pt, align=center, minimum width=4.4cm, minimum height=1.12cm},
    eqbox/.style={draw, rounded corners=1pt, align=center, minimum width=5.1cm, minimum height=1.35cm},
    corrbox/.style={draw, rounded corners=1pt, align=center, minimum width=6.1cm, minimum height=1.8cm}
]

\node[box] (section) at (0,0) {Structural\\cross-section};
\node[eqbox] (mass) at (4.7,0) {Mass law\\[-1pt]
    \(m'_{\mathrm{base}}\)\\[1pt]
    \(R_{w,\mathrm{mass}}, L_{n,w,\mathrm{mass}}\)};
\node[box] (checkbare) at (9.5,0) {Targets\\fulfilled?};
\node[box] (none) at (9.5,2) {No additional\\floor build-up};
\node[widebox] (floating) at (9.5,-2) {Floating floor\\glass wool + screed + parquet};
\node[eqbox] (spring) at (16.5,-1) {Mass-spring-mass\\[-1pt]
    \(f_{0,R}, f_{0,L}\)\\[1pt]
    \(\Delta R_{w,\mathrm{fl}},\Delta L_{n,w,\mathrm{fl}}\)};
\node[corrbox] (damping) at (24.5,-2) {Damping corrections\\[-1pt]
    gravel: \(\Delta_{\mathrm{gravel}}\leq 6\,\mathrm{dB}\)
\((1\,\mathrm{dB}/40\,\mathrm{kg/m^2})\)\\[-1pt]
    hollow-core: \(\Delta_{\mathrm{insulation}}\leq 7\,\mathrm{dB}\)
    \((6\,\mathrm{dB}/0.20\,\mathrm{m})\)\\[-1pt]
    TCC topping: \(\Delta_{\mathrm{concrete}}\leq 6\,\mathrm{dB}\)
\((1\,\mathrm{dB}/40\,\mathrm{kg/m^2})\)};
\node[widebox] (tcc) at (16.5,-3.5) {TCC branch\\topping as mass\\and impact damping};
\node[eqbox] (output) at (32.0,0) {Complete floor assembly\\[-1pt]
    \(R_w=R_{w,\mathrm{mass}}+\Delta R\)\\[1pt]
    \(L_{n,w}=L_{n,w,\mathrm{mass}}-\Delta L\)};

\draw[arrow] (section) -- (mass);
\draw[arrow] (mass) -- (checkbare);
\draw[arrow] (checkbare) -- node[left, font=\small] {yes} (none);
\draw[arrow] (checkbare) -- node[left, font=\small] {no} (floating);
\draw[arrow] (floating) -- (spring);
\draw[arrow] (spring) -- (damping);
\draw[arrow] (floating) -- (tcc);
\draw[arrow] (none) -| node[above, font=\small] {$\Delta R\ = \Delta L = 0$} (output);
\draw[arrow] (damping) -| node[below, font=\small] {$\Delta R\, \ \Delta L$}  (output);
\draw[arrow] (tcc) -- (damping);

\end{tikzpicture}
