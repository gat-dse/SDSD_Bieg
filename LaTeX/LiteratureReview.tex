...
\subsubsection{Cross-section efficiency}
...
\subsubsection{Structural-system efficiency}
The structural efficiency of a slab system is mainly influenced by the span of the system, whether it spans in one or multiple directions, and the continuity over the support. To illustrate this: A two-way spanning slab with continuity exhibits only one-tenth of the deflections compared to a one-way simply supported system, enabling much slender cross-sections \cite{Kaufmann2024_WhitePaper}. Whether continuity is achievable depends heavily on the chosen materialisation, as further elaborated in Section \ref{Sec: FloorSyst}. Furthermore, the span length largely influences the governing design criteria, with internal bending moments scaling proportionally to the span squared and deflections scaling proportionally to the span to the fourth power. Consequently, designers must strike a careful balance between selecting shorter spans to maximise structural and material efficiency, and maintaining adequate spatial flexibility to ensure a long service life for the building \cite{Kaufmann2024_WhitePaper}.


\subsection{Indirect building-level effects \label{Sec: IndirectInfl}}
...
The influence of designers’ decisions and responsibilities extends to further dimensions. While structural loading is primarily governed by the intended use of a building, designers should critically review and discuss the selected loading scenario. For example, heavy earth cover, excessive build-ups, or unnecessarily high live-load assumptions should be avoided where they are not justified by the intended use. Structures are often designed for higher load scenarios to preserve future flexibility. Similarly, non-continuous systems are used for easier deconstruction for circularity purposes.
...
\subsection{Construction cost \label{Sec: ConstructionCost}}
The construction sector is a cost-driven environment, as competitive bidding is standard practice. Economically uncompetitive solutions will not lead to widespread use. Therefore, for a practice-oriented comparison, costs should also be taken into account to ensure financially feasible designs.
...
\newpage
\subsection{Solid reinforced concrete slab \label{Sec: LR_RecConc}}
...
In current Swiss building practice, the advantages of solid concrete slabs named in Section \ref{Sec: LR_RecConc} remain significant.
Thus, post-tensioned solid concrete slabs with unbonded tendons were included in the investigation as a system that combines the advantages of a solid slab with the structural efficiency provided by prestressing \cite{Rombach2010}.
...
Both effects improve serviceability behaviour with respect to deflections and enable longer spans or reduced slab thicknesses compared with conventional reinforced concrete slabs. 

Consequently, in the context of sustainability-optimised floor systems, post-tensioned slabs represent a major competitor, as they can reduce the required quantities of concrete and reinforcing steel while simultaneously minimising the overall building height. For point supports in particular, the punching behaviour is improved. The applied action is reduced by tendon deviation forces, while the resistance is increased through reduced slab rotations \cite{Rombach2010}.
...
\begin{figure}[htbp]
    \centering
    \resizebox{\textwidth}{!}{%
        \input{IMAGES/tikz/tendon_layout}
    }
...
These composite slabs combine the strengths of two distinct materials. Concrete is placed in the upper compression zone to provide stiffness, fire protection and mass, while timber is used in the lower tension zone. Consequently, less reinforcement and concrete are required compared to a conventional reinforced concrete system, while acoustic insulation, improved vibration behaviour and fire protection are ensured by the concrete topping \cite{proHolz2012_Zuschnitt45}. The degree of composite action is governed by the stiffness of the shear connection between the two materials \cite{Eurocode5_2023}.
...
\section{Identified sustainability assessment criteria}
The literature review can be concluded by identifying the factors that serve as comparison criteria for evaluating the sustainability performance of floor systems. Sustainability is understood here as comprising environmental, social, and economic dimensions. 

\newpage

Environmental sustainability is evaluated through the GWP, mass and height of the slab system. Mass and height represent indirect influences at the building level. The economic dimension is considered through cost and construction time. To account for comfort requirements related to acoustic insulation, the assessment is carried out both for the structural slab alone and for the complete floor system, including the floor build-up required to meet acoustic insulation requirements. 

Based on the current market situation and on systems that promise improved ecological sustainability, the following slab systems are identified as relevant for multi-criteria sustainability optimisation and comparison: 
Solid concrete slab, ribbed concrete slab, post-tensioned solid concrete slab, solid timber slab, hollow-core timber slab and timber--concrete composite slab. The requirements that all systems must fulfil are listed in Table \ref{Tab: ReqSS}.
...
\begin{table*}[h]
    ...
    \begingroup
    \scriptsize
    \caption{Sustainablility assessment criteria used for the slab-system comparison. \label{Tab: SustFact}}
    \setlength{\tabcolsep}{4pt}
    \renewcommand{\arraystretch}{1.2}
    \begin{tabularx}{\textwidth}{@{}L{2.6cm}|L{3.5cm}|L{1.6cm}|X@{}}
    \hline
    \textbf{Name} &
    \textbf{Parameter} &
    \textbf{Unit} &
    ...
    m &
    Section \ref{Sec: IndirectInfl}: Total height.  \\

    Mass &
    $m_{\mathrm{struct}}$, $m_{\mathrm{tot}}$ &
    kN/m$^2$ &
    Section \ref{Sec: IndirectInfl}: Total mass.  \\
...
\begin{table*}[h]
    ...
    \begingroup
    \scriptsize
    \caption{Requirements according to the Swiss national standard that must be fulfilled \cite{SIA181_2020, SIA260_2013}. \label{Tab: ReqSS}}
    \vspace{-0.5cm}
    \setlength{\tabcolsep}{4pt}
    \renewcommand{\arraystretch}{1.2}
    \begin{tabularx}{\textwidth}{@{}L{2.4cm}|L{3.8cm}|X@{}}
    \hline
    \textbf{Requirement} &
    \textbf{Limit} &
    ...
    Automated generation of the floor build-up to fulfil the selected acoustic requirement level, including airborne and impact sound checks \cite{SIA181_2020}. 
    The assumed planning surcharges account for flanking transmission and were discussed with a building physics expert. Timber systems were assigned a higher planning surcharge due to the connections complexity. \\ \hline
    \end{tabularx}
    \endgroup
\end{table*}
