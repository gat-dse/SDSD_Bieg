...
Thus, post-tensioned solid concrete slabs with unbonded tendons were included in the investigation as a system that combines the advantages of an inefficient solid slab with the structural efficiency provided by prestressing. \cite{Rombach2010}
...
For point supports, the punching behaviour is improved. The applied action is reduced by tendon deviation forces, while the resistance is increased through reduced slab rotations \cite{Rombach2010}.
...
The literature review can be concluded by identifying the factors that serve as comparison criteria for evaluating the sustainability performance of floor systems. Sustainability is understood here as comprising environmental, social, and economic dimensions. Environmental sustainability is evaluated through the GWP, mass and height of the slab system. Mass and height represent indirect influences at the building level. The economic dimension is considered through cost and construction time. To account for comfort requirements related to acoustic insulation, the assessment is carried out both for the structural slab alone and for the complete floor system, including the floor build-up required to meet acoustic insulation requirements. 

Based on the current market situation and on systems that promise improved ecological sustainability, the following slab systems are identified as relevant for multi-criteria sustainability optimisation and comparison. The requirements that all systems must fulfil are listed in Table \ref{Tab: ReqSS}.

\begin{itemize}
    \item Solid rectangular concrete slab
    \item Ribbed concrete slab
    \item Post-tensioned solid rectangular concrete slab
    \item Solid rectangular timber slab
    \item Hollow-core timber slab
    \item Timber--concrete composite slab
\end{itemize}
...
\begin{table*}[h]
    ...
    The assumed planning surcharges account for flanking transmission and were discussed with a building physics expert. For timber systems, a higher planning surcharge is assumed due to the increased complexity of the connections. \\ \hline
    \end{tabularx}
    \endgroup
